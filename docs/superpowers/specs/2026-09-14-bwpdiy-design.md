...
1. 卡图按 `rotate/scale/offset` 变换（旋转绕中心、画布扩展、加载即缓存；v1.2.1）→ 叠加牌框（卡图区天然透明）→ 裁掉框外内容（v1.0 起蒙版废弃）
...
- **右栏**：表单，按卡牌类型动态显隐字段（含觉醒勾选框，仅战斗/法术/形态/幻境），即时校验（口径对齐 store schema），保存 422 错误表单侧展示；卡图上传（选图 → 落盘项目 `images/` 并写回 `artwork.images[0].path`）与卡图定位 offset_x/offset_y/scale/rotate 数字输入（v1.2.1 起支持旋转；不做拖拽/滚轮定位）
