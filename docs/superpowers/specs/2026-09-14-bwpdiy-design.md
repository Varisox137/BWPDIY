...
4. 文本：卡名（田氏颜体大字库 2.0）+ 描述（方正北魏楷书）自动排版——从大到小试字号、自动换行、逐行居中；文字颜色按框品；`[[关键词]]` 异色高亮（v1.2，双括号标记+配对校验）与 `#xx` 内嵌图标（v1.3.0，ICON_CODES 拼音首字母映射、派系墨染变体、相邻文字四分之一宽空格、宽度计入换行/居中、大小=字号×desc 区 `icon_scale` 系数）均已落地
