...
| 术语 | 代码标识 | 说明 |
|---|---|---|
| 卡图 | artwork | 用户导入的插画，经变换后叠加于牌框之下；一张卡可有原画+多张异画；变换字段 `offset_x`/`offset_y`/`scale`/`rotate`（v1.2.1，绕图片中心顺时针度数、画布自动扩展，加载即缓存） |
