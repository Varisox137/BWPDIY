...
| 描述文本 | desc_text | 方正北魏楷书，自动排版（从大到小试字号、自动换行、逐行居中、文本块竖直居中） |
...
| 文本区 | 命名矩形区域（仅 desc：`center`+`width`+`height`，可选 `obstacle_gap` 避让间距、缺省 4），自动换行、逐行居中、文本块竖直居中（先定字号与行数，再把文本块中心对齐区域中心）、字号递减适配；显式 `\n` 强制断行（连续 `\n` 合并为一个，每段内再自动换行）；激活的 stat 元素（0 值不渲染的 stat 不参与）在透明层单独渲染取 alpha **真实墨迹**作避让掩膜（非组件矩形 bbox）：每个描述行 y 带内被占用的 x 区间向外扩 `obstacle_gap` 后逐行收窄，保证描述墨迹与角标墨迹相距 ≥ gap（y 带同样竖直外扩 gap 查询） |
