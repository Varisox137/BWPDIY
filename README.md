...
- [x] v1.2.3：式神上限 49（卡牌 299 不变）；左栏拆分「主要式神」与「卡牌」两栏分别计数；
  新建卡牌默认类型式神、手动填写输入框加提示文本；拆除旧版 shikigami.yaml 惰性迁移（无实际用户）
- [x] v1.2.4：卡名字体升级田氏颜体大字库 2.0（鸮/雫/鵺等生僻字补字区由细宋体换为颜体楷书，风格统一）
