...
1. 卡图按 `scale/offset` 变换 → 叠加牌框（卡图区天然透明）→ 裁掉框外内容（v1.0 起蒙版废弃）
...
4. 文本：卡名（田氏颜体）+ 描述（方正北魏楷书）自动排版——从大到小试字号、自动换行、逐行居中；文字颜色按框品；`[关键词]` 异色高亮已落地（v1.2，括号配对校验）；`#xx` 内嵌图标二期实现
...
- **右栏**：表单，按卡牌类型动态显隐字段（含觉醒勾选框，仅战斗/法术/形态/幻境），即时校验（口径对齐 store schema），保存 422 错误表单侧展示；卡图上传（选图 → 落盘项目 `images/` 并写回 `artwork.images[0].path`）与卡图定位 offset_x/offset_y/scale 数字输入（不做旋转/拖拽/滚轮定位）
