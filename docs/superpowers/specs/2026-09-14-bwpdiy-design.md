...
└── <项目名>/                # 一个式神项目（v1.2.2 起对应 BWPro 大版本，可多式神）
    ├── shikigami/
    │   └── <任意文件名>.yaml  # 式神卡，数量不限，卡名以文件内 name 为准
    ├── cards/
    │   └── <任意文件名>.yaml  # 非式神卡，单项目上限 299 张
    └── images/              # 卡图原图（上传落盘 hash 命名，见 §5）
```
...
4. 文本：卡名（田氏颜体）+ 描述（方正北魏楷书）自动排版——从大到小试字号、自动换行、逐行居中；文字颜色按框品；`[[关键词]]` 异色高亮已落地（v1.2，双括号标记+配对校验）；`#xx` 内嵌图标二期实现
...
- **左栏**：项目列表 + 项目内卡牌列表（式神组 `式神·<name>` 在前，卡组计数 `n/299`；v1.2.2 起：新建项目为空项目，式神卡数量不限、可删；卡名在表单直接编辑，不再提供「重命名卡牌」按钮——文件名与卡名脱钩，GUI 不改文件名；式神改名时服务端联动更新同项目引用并提示）
- **中栏**：预览——编辑期走项目卡预览 + 表单整体覆盖（含未保存修改与卡图 offset/scale，基准目录 = 项目 `images/`）debounce 实时刷新；选中/保存后走项目卡预览（读盘，缺图回退占位）
- **右栏**：表单，按卡牌类型动态显隐字段（含觉醒勾选框，仅战斗/法术/形态/幻境），即时校验（口径对齐 store schema），保存 422 错误表单侧展示；所属式神为下拉（「（手动填写）」+ 同项目式神名列表，协战为式神1/2 两个）；卡图上传（选图 → sha256 前 16 位 hash 命名落盘项目 `images/`，同图去重不清理旧图；写回 `artwork.images[0].path`）与卡图定位 offset_x/offset_y/scale/rotate 数字输入（v1.2.1 起支持旋转；不做拖拽/滚轮定位）
