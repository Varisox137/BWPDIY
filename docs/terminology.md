...
定稿日期：2026-09-14。修订：v1.0（2026-09-18，PSD 素材替换/四框品/新合成管线/扩展选项）；v1.2（描述文本关键字高亮）；v1.3.0（描述文本 `#xx` 内嵌图标）；v1.4.0（标记扩展：勾玉六色/式神等级/派系样式/N 稀有度合并；数值变色与多位数逐字拼接）。新增/变更术语须同步本表。
