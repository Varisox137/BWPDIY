...
墨迹区间向 x 两侧外扩 gap、行 y 带向竖直两侧只外扩 VERTICAL_GAP=1px
（横向避让保持 obstacle_gap，竖向收紧到 1px 给文本更多可用行——
贴邻行的文本不再因远处角标被无谓收窄）。
"""

import math

# 行 y 带竖直外扩量（px）：只防竖直贴脸，横向间距仍由 obstacle_gap 保证
VERTICAL_GAP = 1
...
def clamp_span_by_mask(span: tuple[float, float], y: float, half_h: float,
    ...
    """行 y 带 [y-half_h, y+half_h] 竖直外扩 VERTICAL_GAP 后与掩膜墨迹相交时按侧收窄 span。
    ...
    left, right = span
    center = (left + right) / 2
    y0 = math.floor(y - half_h - VERTICAL_GAP)
    y1 = math.ceil(y + half_h + VERTICAL_GAP)
