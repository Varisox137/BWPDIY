...
- [x] v1.4.1：协战双式神框渲染部件（`duo_frame` 开关，双菱形头像自动取所属式神卡图）、式神卡 `portrait` 头像设置段（变换+头像框开关）与卡图下方 2 倍头像预览、「导出头像」按钮（数字输入框整体收窄）；标记扩展开放等级 0/稀有度「无」（衍生牌口径）
- [x] v1.4.2：单卡布局覆盖（卡面 `layout` 稀疏覆盖段，卡牌库「调整单卡元素布局」进入单卡模式，剔同写盘）、改名同步重命名配置文件（无 id 且按卡名命名时）、全栏目未保存切换提示
